...
def analyze_layout():
    ...
    formUrl = "https://download.microsoft.com/download/3/6/B/36B6331E-0C63-4E71-A05D-EE88D05081F8/surface-hub-user-guide-en-us.pdf"
    ...
    if result.figures:                    
        for figures_idx, figures in enumerate(result.figures):
            ...
            try:
                ...
                image_data = next(document_intelligence_client.get_analyze_result_figure(model_id=result.model_id, result_id=poller.details["operation_id"], figure_id=figures.id))
                # Load image from BytesIO
                im = Image.open(BytesIO(image_data))
                # Display image
                im.show()
                # Save the image to 'result.FORMAT', using the image format
                im.save(f"FigureId_{figures.id}.{im.format}")
            except OSError as e:
                print(e)
